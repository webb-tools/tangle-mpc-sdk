...
function renderPage(page) {
    ...
    page.sections.forEach((section) => {
        md += `- **[${(0, util_1.stringCamelCase)(section.name)}](#${(0, util_1.stringCamelCase)(section.name).toLowerCase()})**\n\n`;
    });
    // contents
    page.sections.forEach((section) => {
        md += '\n___\n\n\n';
            ...
            Object
                ...
                versions
                    ...
                    Object
                        ...
                        container.items.push({
                            interface: '`' + `api.call.${(0, util_1.stringCamelCase)(apiName)}.${(0, util_1.stringCamelCase)(methodName)}` + '`',
                            name: `${(0, util_1.stringCamelCase)(methodName)}(${args}): ${'`' + type + '`'}`,
...
function addConstants(runtimeDesc, { lookup, pallets }) {
    return renderPage({
        ...
        sections: pallets
            ...
            .map(({ constants, name }) => {
            const sectionName = (0, util_1.stringLowerFirst)(name);
            return {
                items: constants
                    .sort(sortByName)
                    .map(({ docs, name, type }) => {
                    const methodName = (0, util_1.stringCamelCase)(name);
                    return {
...
function addStorage(runtimeDesc, { lookup, pallets, registry }) {
    ...
    const moduleSections = pallets
        ...
        .map((moduleMetadata) => {
        const sectionName = (0, util_1.stringLowerFirst)(moduleMetadata.name);
        return {
            items: moduleMetadata.storage.unwrap().items
                ...
                const methodName = (0, util_1.stringLowerFirst)(func.name);
                const outputType = (0, StorageKey_1.unwrapStorageType)(registry, func.type, func.modifier.isOptional);
                return {
        ...
        sections: moduleSections.concat([{
                ...
                items: Object.entries(substrate).map(([name, { meta }]) => {
                    const arg = meta.type.isMap
                        ...
                        : '';
                    const methodName = (0, util_1.stringLowerFirst)(name);
                    const outputType = (0, StorageKey_1.unwrapStorageType)(registry, meta.type, meta.modifier.isOptional);
                    return {
    ...
    return renderPage({
        ...
        sections: pallets
            ...
            items: lookup.getSiType(meta.events.unwrap().type).def.asVariant.variants
                ...
                return {
                    interface: '`' + `api.events.${(0, util_1.stringCamelCase)(meta.name)}.${methodName}.is` + '`',
                    name: `${methodName}(${args})`,
                    ...(docs.length && { summary: docs })
                };
            }),
            name: (0, util_1.stringCamelCase)(meta.name)
        })),
        title: 'Events'
    });
}
    ...
    return renderPage({
        ...
        sections: pallets
            ...
            items: lookup.getSiType(moduleMetadata.errors.unwrap().type).def.asVariant.variants
                .sort(sortByName)
                .map((error) => ({
                interface: '`' + `api.errors.${(0, util_1.stringCamelCase)(moduleMetadata.name)}.${error.name.toString()}.is` + '`',
                name: error.name.toString(),
                ...(error.docs.length && { summary: error.docs })
            })),
            name: (0, util_1.stringLowerFirst)(moduleMetadata.name)
        })),
        title: 'Errors'
    });
}
